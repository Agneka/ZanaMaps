...
public class GestionDB {
    ...
    public void insertar(Punto punto){
        ...
        String select ="SELECT COUNT(*) FROM Puntos;";//select del contador de puntos existentes para generar siguiente id(autoincremental)
        Cursor c=db.rawQuery(select,null);//cursor que contiene las filas de la consulta select
        c.moveToFirst();//como el cursor solo tiene una fila no hace falta hacer un bucle
        String id=c.getString(0);//cogemos el primer y unico resultado de la fila que sera el id del punto a insertar
        punto.setId(Integer.parseInt(id));//le damos el id al objeto punto
    ...
    public ArrayList recuperarPuntos(){
        ...
        if (c.moveToFirst()){//si el cursor contiene datos
            ...
            while (i< c.getCount()){
                ...
                if (atributos[4] == "0") {//si es cero el atributo visitado sera false
    ...
    public void marcarVisto(Punto punto){
        String update="UPDATE Puntos SET visitado=1 where id=" + punto.getId() + ";";//sentencia update del punto dado
        db.execSQL(update);//ejecutar update
    }
}
